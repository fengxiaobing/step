...
import com.example.step.App;
import com.example.step.R;
import com.example.step.activity.Main2Activity;
...
public class StepService extends Service implements SensorEventListener {
    ...
    //数据库
    private StepEntityDao stepDataDao;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
         ...
         * 总之有需要的可以和我交流
         */
        nfIntent = new Intent(this, Main2Activity.class);
        builder
...
//                        Message replyMsg = Message.obtain(null, Constant.MSG_FROM_SERVER);
                        Message replyMsg = Message.obtain(null, 0x12);
    ...
    private void initTodayData() {
        ...
        stepDataDao = session.getStepEntityDao();
        ...
        //查询数据库中的数据
        //获取当天的数据，用于展示
        StepEntity entity = stepDataDao.queryBuilder().where(StepEntityDao.Properties.CurDate.eq(CURRENT_DATE)).unique();
    ...
    private void saveStepData() {
        ...
        if (entity == null) {
            ...
            stepDataDao.insertOrReplace(entity);
        } else {
            //有则更新当前的数据
            entity.setSteps(String.valueOf(CURRENT_STEP));

            stepDataDao.update(entity);
        }
        //获取一个Notification构造器
        builder = new Notification.Builder(this.getApplicationContext());
        notificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
